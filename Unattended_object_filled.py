    ...
    for i in range(len(detections)):
        ...
        if check_unattended_objects(unattended_objects, class_id, timestamp):
            if class_name not in unattended_colors:
                ...
                # Add 50% opacity to the color
                color_with_opacity = color + (0.5,)
                unattended_colors[class_name] = color_with_opacity
            ...
            annotated_frame = cv2.rectangle(annotated_frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), unattended_colors[class_name], -1)
